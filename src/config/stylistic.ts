...
export async function stylistic(options: StylisticConfig & OptionsOverrides): Promise<Config> {
  ...
  const customizeConfig = pluginStylistic.configs.customize(({ pluginName: 'style', indent, jsx, quotes, semi }))

  return { plugins: { antfu: pluginAntfu, style: pluginStylistic }, rules: {
